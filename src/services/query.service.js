...
export class QueryEngine {
    ...
    async exec({ includeFilterSchema = false, includeDistinctFilterValues = false } = {}) {
        ...
        if (includeFilterSchema) {
            ...
            return { ...result, filterSchema: schema, _internalQuery: finalQueryObject, _searchQuery: searchQuery };
        }
    ...
    _buildAll() {
        ...
        const selectFieldsFromQuery = fields ? fields.split(",").map(f => f.trim()).filter(Boolean) : [];

        const selectFieldsFromOptions = this.options.select ? this.options.select.split(" ").filter(Boolean) : [];
    ...
    _applySorting(query, sortParam) {
        ...
        const clauses = String(sortParam).split(",").map(s => s.trim()).filter(Boolean);
        if (!clauses.length) return query;
        const sortObj = {};
        for (const c of clauses) sortObj[c.startsWith("-") ? c.slice(1) : c] = c.startsWith("-") ? -1 : 1;
    ...
    _autoDetectStringFields() {
        ...
        const available = Object.keys(this.schemaInfo).filter(k => this.schemaInfo[k].type === "string");
        return [...candidates.filter(c => available.includes(c)), ...available.filter(a => !candidates.includes(a))];
    }

    _castValue(raw, type) {
        if (raw == null) return undefined;
        if (type === "number") return isNaN(Number(raw)) ? undefined : Number(raw);
        if (type === "boolean") return raw.toString().toLowerCase() === "true" ? true : raw.toString().toLowerCase() === "false" ? false : undefined;
        if (type === "date") { const d = new Date(raw); return isNaN(d.getTime()) ? undefined : d; }
        return raw;
    }
